...
int utility(uint64_t board_r, uint64_t board_y) {
    if (is_win(board_r)) return 1000000;
    if (is_win(board_y)) return -1000000;
    return 0;
}
    ...
    for (int i = 0; i < HEIGHT; i++) {
        for (int j = 0; j < WIDTH; j++) {
            ...
            while (!get_bit(opp_board, i, j+adder) && j+adder < WIDTH) {
                if (hoz_cache[i][j+adder]) break; // this is a square another 4-in-a-row uses
                hoz_cache[i][j+adder] = 1;
                cnt++;
                adder++;
            }
    ...
    for (int i = 0; i < HEIGHT; i++) {
        for (int j = 0; j < WIDTH; j++) {
            ...
            while (!get_bit(opp_board, i+adder, j) && i+adder < HEIGHT) {
                if (ver_cache[i+adder][j]) break; // this is a square another 4-in-a-row uses
                ver_cache[i+adder][j] = 1;
                cnt++;
                adder++;
            }
            if (cnt >= 4) num_of_rows+=2*mod;
        }
    }
    ...
    for (int i = 0; i < HEIGHT; i++) {
        for (int j = 0; j < WIDTH; j++) {
            ...
            while (!get_bit(opp_board, i+adder, j+adder) && j+adder < WIDTH && i+adder < HEIGHT) {
                if (cache[i+adder][j+adder]) break; // this is a square another 4-in-a-row uses
                cache[i+adder][j+adder] = 1;
                cnt++;
                adder++;
            }
    ...
    for (int i = HEIGHT-1; i >= 0; i--) {
        for (int j = 0; j < WIDTH; j++) {
            ...
            while (!get_bit(opp_board, i-adder, j+adder)  && j+adder < WIDTH && i-adder >= 0) {
                if (cache2[i-adder][j+adder]) break; // this is a square another 4-in-a-row uses
                cache2[i-adder][j+adder] = 1;
                cnt++;
                adder++;
            }
...
{
    if (decision != -1) std::cout << decision << std::endl << std::flush;
    exit(0);
}
